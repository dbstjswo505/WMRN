...
"""
from .DH_pretrain import HeroForPretraining
...
class HeroForVcmr(HeroForPretraining):
    ...
    def get_pred_from_raw_query(self, frame_embeddings, c_attn_masks,
        ...
        if self.lw_neg_ctx != 0 or self.lw_neg_q != 0:
            q2video_scores, score_idx,q2v_score_all,q2v_positive_scores = self.get_video_level_scores(
                modularized_query, frame_embeddings, c_attn_masks,
                val_gather_gpus,out_full=True)
        else:
            q2video_scores = None
        return q2video_scores, st_prob, ed_prob, score_idx, q2v_score_all, q2v_positive_scores
