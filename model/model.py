...
class HierarchicalVlModel(VideoPreTrainedModel):
    ...
    def forward(self, batch, task='repr', compute_loss=True,train=True):
        batch = defaultdict(lambda: None, batch)
        if task == 'repr':
            # get frame representations
            return self.forward_repr(batch, train=train)
    ...
    def forward_repr(self, batch, encode_clip=True, train=True):
        ...
        if train:
            ...
            q = 0.1
        if p >= 0.5:
            B,L,D = transformed_c_v_feats.shape
            LMS = 1
            LME = 3
            SM = int(3*L/4)
            #SM = L - LME
            video_mask = torch.ones(transformed_c_v_feats.shape,device=transformed_c_v_feats.device,dtype=transformed_c_v_feats.dtype)
            start_m = torch.randint(0,SM,(1,))
            length_m = torch.randint(LMS,LME,(1,))
            end_m = start_m + length_m
            video_mask[:,start_m:end_m,:] = 0
            transformed_c_v_feats = transformed_c_v_feats * video_mask
        if q >= 2:
