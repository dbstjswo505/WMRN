...
from .modeling_utils import mask_logits_plus
...
class HeroForPretraining(HeroModel):
    ...
    def forward(self, batch, task='vsm', compute_loss=True):
        ...
        if task == 'vsm':
            frame_embeddings,is_flip = self.v_encoder(batch, 'repr')
            ...
            if self.lw_neg_ctx != 0 or self.lw_neg_q != 0:
                q2video_scores,_,q2v_score_all,q2v_positive_scores = self.get_video_level_scores(
            ...
            if compute_loss:
                ...
                if st_prob is not None:
                    ...
                    _,eidx = torch.max(ed_prob,dim=1)
                    loss_mine = F.cross_entropy(st_prob,eidx.long(), reduction=reduction, ignore_index=-1)
                ...
                if q2video_scores is not None:
                    ...
                    loss_hard_neg = self.get_hard_negative_video_level_loss(
                        q2video_scores, q2v_positive_scores,reduction) 
                ...
                loss_neg_hard = self.lw_neg_ctx * loss_hard_neg

                return loss_st_ed, loss_neg_ctx, loss_neg_q, loss_neg_hard
                #return loss_st_ed, loss_neg_ctx, loss_neg_q, tsp_n
    ...
    def get_hard_negative_video_level_loss(self, query_context_scores, q2v_positive_pair, reduction="mean"):
        ...
        """
        # This is for Positive score
        bsz_q, bsz_v = query_context_scores.size()  # (Nq, Nv)
        num_q_per_v = int(bsz_q/bsz_v)
        loss_hard_neg = torch.tensor(0).to(query_context_scores.device)
        if bsz_v == 1:
            return loss_hard_neg
        ...
        # This is for Hard negative sampling
        q2vp = q2v_positive_pair.clone()
        bsz_q, L = q2vp.shape
        #hard_negative_,_ = torch.sort(hard_negative_,dim=1)
        ...
        # get negative sample from positive q2v pair
        # (Nq,10)
        samples = 5
        ...
        hard_pool_size = int(samples/2)
        if self.use_hard_negative:
            weighting_mat = torch.ones_like(loss_hard_neg)
            weighting_mat[:, hard_pool_size:] = self.hard_neg_weight
            weighting_mat[:, :hard_pool_size] = .1
            loss_hard_neg = weighting_mat * loss_hard_neg
        
        if reduction == "sum":
            return loss_hard_neg.mean(1)
        elif reduction == "mean":
            return loss_hard_neg.mean(1).mean(0)
    ...
    def get_hard_neg_scores(self, scores_masked,
                           pos_indices=None, sample_idx=1):
        """
        ...
        neg_scores = sorted_scores_masked[:, :sample_idx]
        return neg_scores


    def get_hard_pos_scores(self, scores_masked,
                           pos_indices=None, sample_idx=1):
        """
        scores: (N, N), cosine similarity [-1, 1],
            Each row are scores: query --> all videos.
        ...
        masked_score = scores_masked.max()
        sub_mask = (scores_masked == masked_score)*masked_score

        scores_masked_sub = scores_masked-(sub_mask*2)

        sorted_scores_masked, sorted_scores_indices = torch.sort(
            scores_masked_sub, dim=1, descending = True)
        # skip the masked positive
        # (N, sample_size-sample_min_idx)
        pos_scores = sorted_scores_masked[:, :sample_idx]
        return pos_scores
    ...
    def get_video_level_scores(self, modularized_query,
        ...
        tmp = query_context_scores
        query_context_scores = mask_logits(
            query_context_scores, context_mask)  # (N, L, N)
        query_context_scores_plus = mask_logits_plus(
            tmp, context_mask)
        #query_context_scores_plus = query_context_scores_plus[
        B1,L,B2 = query_context_scores_plus.shape
        idx = torch.arange(B1)
        query_context_positive_pair = query_context_scores_plus[idx,:,idx]
    
        query_context_scores_all = query_context_scores
        query_context_scores, score_idx = torch.max(
        ...
        sorted_tmp_query_context_scores, _ = torch.sort(tmp_query_context_scores_all, dim=1, descending=True)
        sorted_query_context_scores = torch.mean(sorted_tmp_query_context_scores[:,:nosm,:],dim=1)
        query_context_scores = sorted_query_context_scores

        ########################################################################

        if not out_full:
            return query_context_scores, score_idx
        elif out_full:
            return query_context_scores, score_idx, query_context_scores_all, query_context_positive_pair
